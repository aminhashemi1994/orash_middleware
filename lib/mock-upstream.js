...
const PACKINGS = [2, 3];        // 2 = کلاف (1 does not exist in production)
const MAIN_GROUPS = [1, 2, 10];
const CATEGORIES = [1, 2, 3];
/** Present at all? Absent optional codes are simply not checked. */
...
function createGood(data) {
  ...
  // The order the live service was measured to use on 2026-08-27, because the
  // panel's "is it already registered?" probe depends on it: unit, main group,
  // sub-group, duplicate code, duplicate name, packing, category. Reference
  // codes are checked only when present.
  if (!name) return itemFail('مقدار فيلد عنوان کالا و خدمات نمي تواند خالي باشد .');
  if (data.type === undefined || data.type === '') return itemFail('نوع کالا يا خدمات وارد نشده است .');
  if (data.unitIdRef === undefined || data.unitIdRef === '') {
    return itemFail('کد واحد اندازه گيري وارد نشده است .');
  }
  if (!UNITS.includes(Number(data.unitIdRef))) return itemFail('کد واحد اندازه گيري وارد شده صحيح نيست .');
  if (has(data.mainGroupCodeRef) && !MAIN_GROUPS.includes(Number(data.mainGroupCodeRef))) {
    return itemFail('کد گروه اصلي صحيح نيست .');
  }
  if (has(data.secondGroupCodeRef) && !SECOND_GROUPS.includes(Number(data.secondGroupCodeRef))) {
    return itemFail('کد گروه فرعي صحيح نيست .');
  }
  if (data.code && goods.some((g) => String(g.code) === String(data.code))) {
    return itemFail('مقدار فيلد کد کالا و خدمات تکراري است .');
  }
  if (goods.some((g) => g.name === name)) return itemFail('مقدار فيلد عنوان کالا و خدمات تکراري است .');
  if (has(data.unitPackingCodeRef) && !PACKINGS.includes(Number(data.unitPackingCodeRef))) {
    return itemFail('کد نوع بسته بندي وارد شده صحيح نيست .');
  }
  // Last check before the good is created — what the panel's probe relies on.
  if (has(data.goodCategoryIdRef) && !CATEGORIES.includes(Number(data.goodCategoryIdRef))) {
    return itemFail('کد طبقه بندي صحيح نيست .');
  }
  ...
  goods.push({
    ...
    secondGroupName: data.secondGroupCodeRef == null || data.secondGroupCodeRef === ''
      ? '' : 'زیرگروه ' + data.secondGroupCodeRef,
