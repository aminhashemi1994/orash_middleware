...
function createGood(data) {
  ...
  // Optional, like the packing code: a label omits it until the second group
  // arrives by name and can be resolved against the panel.
  if (data.secondGroupCodeRef != null && data.secondGroupCodeRef !== ''
      && !SECOND_GROUPS.includes(Number(data.secondGroupCodeRef))) {
