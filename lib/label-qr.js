...
// main group 1 is what every label carries for now.
//
// Two fields are deliberately absent rather than zero. `unitPackingCodeRef` is
// optional in CreateGood (no asterisk in the PDF's parameter table) and its real
// code is not known yet — and 0 is not a valid packing code, so sending it risks
// a rejection the field never had to cause. `secondGroupCodeRef` will later
// arrive by name and be resolved against the panel; note the PDF *does* mark it
// required, so if the service rejects a label for a missing sub-group, that is
// the field to give a real code to.
const FIXED = {
  type: 1,
  unitIdRef: 5,
  mainGroupCodeRef: 1,
