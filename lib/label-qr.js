...
/**
 * @param {{code:string, serial:string, name:string, color?:string,
 *   lengthValue?:string|number}} fields  straight from the sheet
 * @returns {{code:string, name:string, serial:string, color?:string,
 *   lengthValue?:number}} the QR content
...
function buildLabel(fields) {
  ...
  // Colour is its own field, not part of the name: the sheet keeps it in its
  // own cell, and merging it in made every name end in a colour word that no
  // search for the product itself would match.
  const color = clean(fields.color);
