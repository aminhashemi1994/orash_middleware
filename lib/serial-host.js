...
class SerialHost extends EventEmitter {
  constructor() {
    ...
    this._timer = null;
    this._opening = false;
    // How many panels are listening on the host bridge. The device is held only
    // while this is above zero — see attach().
    this._demand = 0;
  }
  ...
  /**
   * A panel started listening on the host bridge.
   *
   * The device is opened on demand rather than at boot, because holding it is
   * not free: a tty has a single owner, so while this process has /dev/ttyACM0
   * open, Chrome's Web Serial cannot open it and reports only "Failed to open
   * serial port". A panel reached over the network needs the browser path, and
   * the operator's scanner is on the operator's own machine — so a server that
   * grabbed the port at boot would lock out the very path that can still work.
   *
   * Presence detection does not need the port open (lib/serial-check.js only
   * stats /dev and reads /sys), so the panel can still be told a scanner is
   * there before anything is claimed.
   */
  attach() {
    this._demand++;
    if (this._demand === 1) this._startPolling();
    return this;
  }

  /** A panel stopped listening. At zero the device is released. */
  detach() {
    this._demand = Math.max(0, this._demand - 1);
    if (this._demand === 0) {
      clearInterval(this._timer);
      this._timer = null;
      this._close();
      console.log('[serial] no panel is listening — released the device');
    }
    return this;
  }
  ...
  /** Kept for callers that just want the reader ready; a no-op without demand. */
  start() { return this; }
  ...
  /**
   * Bytes stay bytes until a whole code is in hand: the device's encoding is
   * only decidable over a complete payload (see ScanCore.decodeScan), and
   * decoding chunk by chunk would also cut multi-byte characters in half.
   */
  _ingest(chunk) {
    this._buffer = this._buffer.length ? Buffer.concat([this._buffer, chunk]) : Buffer.from(chunk);
    let idx;
    while ((idx = this._buffer.findIndex((b) => b === 0x0D || b === 0x0A)) !== -1) {
      const line = scanCore.decodeScan(this._buffer.subarray(0, idx)).trim();
  ...
  _armFlush() {
    this._flushTimer = setTimeout(() => {
      const text = scanCore.decodeScan(this._buffer).trim();
      if (scanCore.looksTruncated(text) && this._waited < TRUNCATED_WAIT_MS) {
